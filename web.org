...
items that were copied.  This can be enabled by:
...
When this is disabled (default), the Ctrl+v only pastes and
Ctrl+Shift+v cycles through the previous items pasted.

By taking away the shift key, it reduces pinky usage.

When installed, ergoemacs' smart paste will also use browse-kill-ring
