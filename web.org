...
*** Org mode
Ergoemacs-mode respects org-mode's Ctrl+Enter and Alt+arrow keys.
Also pasting is done by org-mode's pasting mechanism instead of the
default CUA-mode's pasting mechanism.
*** Cua-mode
Cua mode is enabled by default and respects ergoemacs-keys.  CUA
rectangle support is enabled by changing the Alt+ keys to Win+ keys.  
