	...
	(setq svg (ergoemacs-theme--svg theme)
	  ...
	  (when ergoemacs-theme--svg-list
	    ...
	    (dolist (elt ergoemacs-theme--svg-list)
	      (insert (ergoemacs-key-description (nth 0 elt)) ":\n")
	      (cond
               ((and (image-type-available-p 'png)
                     (nth 1 elt)
                     (file-exists-p (replace-regexp-in-string "[.]svg\\'" ".png" (nth 1 elt))))
                (insert-image (create-image (replace-regexp-in-string "[.]svg\\'" ".png" (nth 1 elt))))
                (insert "\n"))
               ((and (image-type-available-p 'svg)
		     (nth 1 elt)
                     (file-exists-p (nth 1 elt)))
                (insert-image (create-image (nth 1 elt)))
                (insert "\n")))
	      (when (file-exists-p (nth 1 elt))
		(insert "[svg]")
		(when (looking-back "\\(\\[svg\\]\\)")
		  (help-xref-button 1 'help-url (nth 1 elt))))
	      (when (file-exists-p (replace-regexp-in-string "[.]svg\\'" ".png" (nth 1 elt)))
                (insert " [png]")
                (when (looking-back "\\(\\[png\\]\\)")
                  (help-xref-button 1 'help-url (replace-regexp-in-string "[.]svg\\'" ".png" (nth 1 elt)))))
	      (insert "\n\n")))
          (insert "\n\n")
...
(defun ergoemacs-theme--svg (&optional theme layout full-p reread)
  ...
  (save-excursion
    (let* ((lay (or layout ergoemacs-keyboard-layout))
           ...
           (file-name (expand-file-name (concat theme "-" lay "-" (syumbol-name (ergoemacs-map--hashkey ergoemacs--start-emacs-state-2)) ".svg") file-dir))
           (reread reread)
           (old-theme ergoemacs-theme)
      ...
      (if (and file-name (file-exists-p file-name) (not reread) (or (not full-p) ergoemacs-theme--svg-list))
		...
		(push (list ergoemacs-theme--svg-prefix file-name) ergoemacs-theme--svg-list)
