...
(defvar ergoemacs-describe-key--display-char-cache nil
  "List of characters and fonts and if they display or not.")

(defun ergoemacs-describe-key--display-char-p (char)
  "Determines if CHAR can be displayed."
  (ignore-errors
    (let* (ret
           (buf (current-buffer))
           (face (font-xlfd-name (face-attribute 'default :font)))
           (found (assoc (list face char window-system) ergoemacs-describe-key--display-char-cache)))
      (if found
          (nth 0 (cdr found))
        ...
        ;; (if at all).
        (push (list (list face char window-system) ret) ergoemacs-describe-key--display-char-cache)
        ret))))

(defun ergoemacs-describe-key--unicode-char (char alt-char)
...
`ergoemacs-display-unicode-characters' is nil display ALT-CHAR"
  (if (and ergoemacs-display-unicode-characters (ergoemacs-describe-key--display-char-p char))
      char
    alt-char))

(defun ergoemacs-describe-key--key (key mod)
  "Key description"
  (let ((ret ""))
    (cond
     ((eq key 'deletechar)
     ...
     ((eq key 32)
      (setq ret "Space"))
     ((eq key 127)
      (setq ret (format "%sBackspace" (ergoemacs-describe-key--unicode-char "←" "left"))))
     ((eq key 'escape)
      (setq ret "Esc"))
     ((eq key 'tab)
     ...
     ((eq key 'return)
      (setq ret (format "Enter%s"
                        (ergoemacs-describe-key--unicode-char "↵" ""))))
     ((memq key '(apps menu))
      (setq ret (ergoemacs-describe-key--unicode-char "▤" "Menu")))
     ((eq key 'left)
      (setq ret (ergoemacs-describe-key--unicode-char "←" "left")))
     ((eq key 'right)
      (setq ret (ergoemacs-describe-key--unicode-char "→" "right")))
     ((eq key 'up)
      (setq ret (ergoemacs-describe-key--unicode-char "↑" "up")))
     ((eq key 'down)
      (setq ret (ergoemacs-describe-key--unicode-char "↓" "down")))
     ((eq key 'prior)
      (setq ret "PgUp"))
     ((eq key 'next)
      (setq ret "PgDn"))
...
(defun ergoemacs-describe-key--modifier (mod)
  ...
  (let (ret)
    (cond
     ...
     ((and (eq mod 'meta) 
      ...
      (setq ret (format "%sOpt+" (ergoemacs-describe-key--unicode-char "⌥" "+"))))
     ((and (eq mod 'meta) ergoemacs-display-small-symbols-for-key-modifiers
           (eq system-type 'darwin)
           (or (and (boundp 'mac-alternate-modifier)
                    (eq mac-alternate-modifier 'meta))
               (and (boundp 'ns-alternate-modifier)
                    (eq ns-alternate-modifier 'meta))))
      (setq ret (format "%s" (ergoemacs-describe-key--unicode-char "⌥" "+"))))
     ((and ergoemacs-display-small-symbols-for-key-modifiers (eq mod 'shift))
      (setq ret (ergoemacs-describe-key--unicode-char "⇧" "+")))
     ...
     ((and (eq mod 'super)
           (eq system-type 'windows-nt))
      (setq ret (format "%sWin+" (ergoemacs-describe-key--unicode-char "⊞" "#"))))
     (t
...
(defun ergoemacs-describe-key--add-emacs-modifiers-for-ergoemacs-modifiers (mod)
...
(defun ergoemacs-describe-key (kbd &optional layout)
      ...
      (dolist (key (listify-key-sequence kbd))
        ...
        (cond
         ((and (memq 'control mod) (eq ev ?\[))
          (setq mod (ergoemacs-describe-key--add-emacs-modifiers-for-ergoemacs-modifiers mod)
                ev 'escape))
        ...
        (setq tmp (format "%s%s%s%s"
                          (or (and ergoemacs-display-key-use-face-p "")
                              (and ergoemacs-display-use-unicode-brackets-around-keys (ergoemacs-describe-key--unicode-char "【" "["))
                              "[")
                          (mapconcat #'ergoemacs-describe-key--modifier
                                     mod "")
                          (ergoemacs-describe-key--key ev mod)
                          (or (and ergoemacs-display-key-use-face-p "")
                              (and ergoemacs-display-use-unicode-brackets-around-keys (ergoemacs-describe-key--unicode-char "】" "]"))
                              "]")))
...
(defun ergoemacs-describe-key-kbd (code)
  "Creates `ergoemacs-mode' style description of kbd macro CODE"
  (if (not code) ""
    (save-match-data
      (ergoemacs-describe-key (read-kbd-macro code t)))))
