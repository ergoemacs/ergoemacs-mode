      ...
      (unwind-protect
          (progn
            ...
            (dolist (map (reverse (cdr keymap)))
              (when label
                (ergoemacs-map-properties--label map))
              (if melt
...
"
  (let* ((keymap (ergoemacs-map-properties--keymap-value keymap))
         (map-key (ergoemacs-map-properties--get keymap :map-key))
         (composed (ergoemacs-map-properties--composed keymap force))
         parent ret)
    (when (and force (not (or map-key composed)))
      (ergoemacs-map-properties--label keymap)
      (setq map-key (ergoemacs-map-properties--get keymap :map-key)
...
(defun ergoemacs-map-properties--get (keymap &optional property)
  ...
  (let (tmp)
    (cond
     ...
     ;; ((and (setq tmp (intern (concat "ergoemacs-map-properties--"(substring (symbol-name :map-list) 1))))
     ;;       (fboundp tmp))
     ;;  (ignore-errors (funcall tmp keymap)))
...
(defun ergoemacs-map-properties--default-global-gen ()
  ...
  (dolist (map ergoemacs-map-properties--label-atoms-maps)
    (ergoemacs-map-properties--get (ergoemacs-sv map) :map-list))
  (with-temp-file (ergoemacs-map-properties--default-global-file) 
    (let ((print-level nil)
          (print-length nil)
          keys)
      (goto-char (point-min))
      (insert "(defvar ergoemacs-map-properties--plist-hash)(defvar ergoemacs-map-properties--original-global-map)(declare-function ergoemacs-map-properties--label \"ergoemacs-map\")")
      (ergoemacs-mapkeymap
      ...
      (ergoemacs-map-properties--label ergoemacs-map-properties--original-global-map)
      (ergoemacs-map-properties--put ergoemacs-map-properties--original-global-map :keys keys)
      ;; (message "Keys: %s\nKey::%s" keys
      ;;          (ergoemacs-map-properties--get ergoemacs-map-properties--original-global-map :keys))
      ...
      (message "global-map-list %s" (ergoemacs-map-properties--get global-map :map-list))
      (dolist (map ergoemacs-map-properties--label-atoms-maps)
        (when (ergoemacs-map-properties--key-struct map)
          (insert (format "(when (boundp '%s) (ergoemacs-map-properties--label %s %s))"
                          map map (ergoemacs-map-properties--get (ergoemacs-sv map) :map-key))))))))
...
"
  (if (not (ergoemacs-keymapp keymap)) nil
    ...
    (when (not (ergoemacs-map-properties--key-struct keymap))
      (ergoemacs-map-properties--label keymap))
    (let ((ret (ergoemacs-map-properties--get keymap :prefixes)) ret2)
      (unless ret
        (ergoemacs-mapkeymap nil keymap 'prefix)
        (setq ret (ergoemacs-map-properties--get keymap :prefixes)))
...
(defun ergoemacs-map-properties--put (keymap property value)
  ...
  (if (eq property :label)
      (ergoemacs-map-properties--label keymap value)
    (let ((keymap (ergoemacs-map-properties--keymap-value keymap)))
      (cond
...
KEYMAP can be a keymap or keymap integer key."
  (let (ret
        ...
        map-key)
    (cond
     ((and map-p (not no-hash) (setq ret (ergoemacs-map-properties--get keymap :map-list-hash)))
      (setq map-key (ergoemacs-map-properties--key keymap))
      (setq map-p ret)
      (setq ret nil)
      (dolist (map map-p)
        (when (or (eq keymap (ergoemacs-sv map))
                  (= map-key (ergoemacs-map-properties--key (ergoemacs-sv map))))
          (push map ret)))
      (when (not ret);; Check again...
        (setq ret (ergoemacs-map-properties--map-list keymap t)))
      ret)
     ((and map-p (setq map-key (ergoemacs-map-properties--key keymap)) (integerp map-key))
      (setq map-p (ergoemacs-map-properties--key keymap))
      (dolist (map ergoemacs-map-properties--label-atoms-maps)
        (when (or (eq keymap (ergoemacs-sv map))
                  (equal map-p (ergoemacs-map-properties--key map)))
          (push map ret)))
      (ergoemacs-map-properties--put keymap :map-list-hash ret)
      ret)
     ((integerp keymap)
      (dolist (map ergoemacs-map-properties--label-atoms-maps)
        (when (equal keymap (ergoemacs-map-properties--key map))
          (unless map-p
            (setq map-p map))
          (push map ret)))
  ...
  (let* (sv)
    (cond 
     ...
     (t ;;Label
      (when sv
        (let (omap key)
          (setq key (ergoemacs-map-properties--key sv))
          (ergoemacs-map-properties--label sv key)
...
(defun ergoemacs-map-properties--key (keymap)
...
"
  (if (not (ergoemacs-keymapp keymap) ) nil
    (if (ergoemacs-map-properties--composed-p keymap)
      ...
      (let* ((map keymap)
             (map-key (or map-key (ergoemacs-map-properties--key map)))
             char-table
             indirect-p
             old-plist
...
KEYMAP can be an `ergoemacs-map-properties--key-struct' of the keymap as well."
  (let (map-key ret)
    (ergoemacs-map--setcdr
     (cond
      ((and (ergoemacs-keymapp keymap) (setq map-key (ergoemacs-map-properties--key keymap))
            (integerp map-key))
       ...
       (if (and (not ret) ;; Don't save empty keymaps as original keymaps...
         ...
         (unless ret
           (ergoemacs-map-properties--label keymap map-key)
...
(defun ergoemacs-global-changed-p (key)
  ...
  (let* ((key (or (and (vectorp key) key)
         ...
         (after-changed (ergoemacs-map-properties--get global-map :keys-after-changed))
         keymap k1 k2 ret)
